...
func (state *state) moveProjectiles() {
...
func (enemy *Enemy) takeDamage(amount uint8, state *state) {
...
func (enemy *Enemy) destroyEnemy(state *state) {
	for idx, obj := range *state.Enemies {
		if obj.id == enemy_.id && enemy_.id != 0 {
			if idx == 0 {
...
func (state *state) destroyProjectile(idx int) {
	if idx == 0 {
...
// if true simply skip
func (state *state) willCollide(x, y int32, hitBoxRadius uint8, selfIdx int) (bool, *Enemy) {
	for idx, enemy := range *state.Enemies {
		if idx == selfIdx {
			continue
		}
...
func (state *state) willCollideWithPlayer(radius uint8, x, y int32) bool {
...
func (state *state) AssignID() uint16 {
	state.nextID++
...
func (state *state) initObject(color [3]uint8, scaler [2]int16, x, y, w, h int32, hp int8) {
	rect := sdl.Rect{X: x - w/2, Y: y - h/2, W: w, H: h}
	radius := calcRadius(h, w)
	enemy := Enemy{x: x, y: y, vector: scaler, rect: &rect, color: color, id: state.AssignID(), hitBoxRadius: uint8(radius), hp: hp}
	*state.Enemies = append(*state.Enemies, enemy)
}

func (state *state) initProjectile(color [3]uint8, lifeLength uint16, speed, damage uint8, x, y, w, h int32) {
	rect := sdl.Rect{X: x - w/2, Y: y - h/2, W: w, H: h}
	radius := calcRadius(h, w)
...
func (state *state) initPlayer(x, y int32, speed uint8, texturePath string) {
